...
float grid(float x, float density) {
  return smoothstep(0.01, 0.02, abs(fract(x * density) - 0.5));
}

void main() {
  vec2 uv = vUv * 2.0 - 1.0;
  vec3 base = vec3(0.02, 0.01, 0.03);
  vec3 glow = waveColor * 0.22;
  float lineMask = grid(vUv.x, 14.0) + grid(vUv.y, 10.0);
  base += glow * 0.13 * lineMask;
  float vignette = smoothstep(1.1, 0.7, length(uv));
  base = mix(base * 0.55, base + glow * 0.4, vignette);
  float pulse = sin(time * 0.9 + vUv.y * 6.0) * 0.03;
  base += vec3(pulse);
...
export default function DitherBackground({
  ...
  waveFrequency = 3,
  waveAmplitude = 0.3,
  waveColor = [0.8, 0.4, 0.1], // Orange color
  colorNum = 4,
  pixelSize = 2,
